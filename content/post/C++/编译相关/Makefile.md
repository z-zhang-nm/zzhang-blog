...
给字符串`<list>`中的单词排序（升序），返回排序后的字符串，注意`sort`会去除`<list>`中相同的单词。
...
取字符串`<text>`中第`<n>`个单词（从1开始），如果`<n>`比单词数大，返回空串。

### 7.2.9 取单词串函数wordlist

```
$(wordlist <s>,<e>,<text>)
```
从字符串`<text>`中取出从`<s>`开始到`<e>`的单词串，`<s>`和`<e>`是数字，如果`<s>`比`<text>`中单词数大，返回空串，如果`<e>`大于`<text>`中单词数，那么返回从`<s>`开始到`<text>`结束的单词串。

### 7.2.10 单词个数统计函数words
...
从文件名序列`<names>`中取出目录部分，即最后一个反斜杠之后的部分，如果没有反斜杠就返回`./`。
...
从文件名序列`<names>`中取出非目录部分，即最后一个反斜杠之后的部分。
...
从文件名序列`<names>`中取出各个文件的后缀，若没有后缀，返回空串。
...
从文件名序列`<names>`中取出各个文件名的前缀，若没有前缀，返回空串。
